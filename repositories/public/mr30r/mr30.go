...
func (r *mr30RepoDB) GetMr30(course_year, course_semester string) (*[]Mr30Repo, error) {
	...
	query := "SELECT id,course_year,course_semester,course_no,course_method,course_method_number,day_code,time_code,room_group,instr_group,course_method_detail,day_name_s,time_period,course_room,course_instructor,show_ru30,course_credit,course_pr,course_comment,course_examdate FROM table(adithep.GET_MR30(:param1,:param2))"
...
func (r *mr30RepoDB) GetMr30Year() (*[]Mr30YearRepo, error) {
	...
	query := "SELECT course_year,course_semester from (select a.study_year course_year, a.study_semester course_semester FROM ugb_ru30_daytime a group by a.study_year, a.study_semester order by 1 desc , 2 desc) where rownum < 4 and  course_year <> '2567'"
